...
- **[The Windows Companion](windows-voice-line.md):** the community-hardened Windows port of the Voice Line, contributed by joatsaint: the real fixes from a full Windows build (input handling, the silent CPU-instead-of-GPU trap, running it as a service). On Windows, paste the Voice Line prompt and this companion together. The site's OS toggle serves them combined automatically.
